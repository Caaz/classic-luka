addPlug('Userbase', {
  ...
  'utilities' => {
    ...
    'login' => sub {
      ...
      my $match = 0;
      my $handle = &{$utility{'Core_Utilities_getHandle'}}($_[0]);
      for(@{$lk{data}{plugin}{'Userbase'}{users}{$_[0]}}) {
    ...
    'info' => sub {
      ...
      for(@{$lk{data}{plugin}{'Userbase'}{users}{$_[0]}}) { if(grep /^$_[1]$/i, @{${$_}{nicknames}}) { if((${$_}{currently}) && (${$_}{currently} eq $_[1])) { return $_; } } }
      return {0=>0};
    },
    'getID' => sub {
      ...
      for(@{$lk{data}{plugin}{'Userbase'}{users}{$_[0]}}) {
        if(grep /^$_[2]$/i, @{${$_}{nicknames}}) {
          ...
          if((${$_}{currently}) && (${$_}{currently} eq $_[2])) { 
            my @nicks = ();
            for(@{${$_}{nicknames}}) { push(@nicks, $_) unless(@nicks > 10);  }
            &{$utility{'Fancify_say'}}($handle,$_[1],">>$i: [\x04${$_}{name}\x04] [Access: >>${$_}{access}] [Nicknames: \x04".(join "\x04, \x04", @nicks)."\x04]");
            return 1;
          }
        }
        $i++;
  ...
  'code' => {
    'irc' => sub {
      my %irc = ('irc' => $_[1], 'raw' => $_[2], 'msg' => $_[3], 'name' => $_[4]);
      ...
      if($irc{msg}[1] =~ /^NICK$/i) {
        ...
        for(@{$lk{data}{plugin}{'Userbase'}{users}{$irc{name}}}) {
          if((${$_}{currently}) && (${$_}{currently} eq $nickname)) {
  ...
  'commands' => {
    ...
    '^Nicknames clear$' => {
      'description' => "Clears nicknames for your account.",
      'tags' => ['utility'],
      'code' => sub {
        my $match = 0;
        my $i=0;
        for(@{$lk{data}{plugin}{'Userbase'}{users}{$_[0]}}) {
          if(grep /^$_[2]{nickname}$/i, @{${$_}{nicknames}}) {
            $match++;
            if((${$_}{currently}) && (${$_}{currently} eq $_[2]{nickname})) {
              @{${$_}{nicknames}} = ($_[2]{nickname});
              &{$utility{'Fancify_say'}}($_[1]{irc},$_[2]{where},"Reset nicknames on your account.");
              return 1;
            }
          }
          $i++;
        }
        if(!$match) { &{$utility{'Fancify_say'}}($_[1]{irc},$_[2]{where},"No userbase accounts tied to that nickname."); }
        else { &{$utility{'Fancify_say'}}($_[1]{irc},$_[2]{where},"You don't seem to be logged in at the moment."); }
      }
    },
    '^set Name ([\w\s]+)$' => {
      'description' => "Clears nicknames for your account.",
      ...
      'code' => sub {
        my $name = $1;
        if(length $name > 15) {
          &{$utility{'Fancify_say'}}($_[1]{irc},$_[2]{where},"Too long");
          return 0;
        }
        my $match = 0;
        my $i=0;
        for(@{$lk{data}{plugin}{'Userbase'}{users}{$_[0]}}) {
          if(grep /^$_[2]{nickname}$/i, @{${$_}{nicknames}}) {
            ...
            if((${$_}{currently}) && (${$_}{currently} eq $_[2]{nickname})) {
              $$_{name} = $name;
              &{$utility{'Fancify_say'}}($_[1]{irc},$_[2]{where},"Name set. Probably.");
              return 1;
            }
          }
          $i++;
